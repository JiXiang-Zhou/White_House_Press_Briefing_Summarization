...
def filterout_short_QaA(QaAs, Q_min = 50, A_min = 22):
    ...
    for QaA in QaAs:
        ...
        if length_count(QaA[1]) < Q_min:
            A=False
        elif  length_count(QaA[0]) < A_min:
            Q=False
        
        if A:
            if Q:
                filtered_QaAs.append(QaA)
            else:
                tagging = [QaA[1],QaA[1]]
                filtered_QaAs.append(tagging)
    
    return filtered_QaAs
# In[ ]:
